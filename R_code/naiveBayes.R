...
library(e1071)
nb_model <- naiveBayes(TotalDelay ~ DayofMonth + FlightNum + ActualElapsedTime + CRSElapsedTime + AirTime + Distance + TaxiOut + TaxiIn + CarrierDelay + WeatherDelay + NASDelay + SecurityDelay + LateAircraftDelay, data=train_data, na.action = na.omit)
nb_pred <- predict(nb_model, test_data, type = "class")

naive_bayes <- data.frame(test_data$ID[1:299], nb_pred[1:299])
names(naive_bayes) <- c("ID", "TotalDelay")
write.csv(naive_bayes, "FinalNaive_Bayes.csv", row.names=FALSE)