...
library(nnet)
neu_net <- nnet(TotalDelay ~ DayofMonth + FlightNum + Distance + TaxiOut + TaxiIn + AirTime, data=train_data, size = 1, na.action = na.omit)
perd <- predict(neu_net, test_data)
neu_pred <- predict(neu_net, test_data, type= "class")

neural_net <- data.frame(test_data$ID, neu_pred)
names(neural_net) <- c("ID", "TotalDelay")
write.csv(neural_net, "NeuralNetFinal.csv", row.names=FALSE)