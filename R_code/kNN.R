...
library(kknn)
dataknn <- kknn(TotalDelay ~ DayofMonth + FlightNum + Distance + TaxiIn + TaxiOut + AirTime,train_data, test_data, k=2, distance=1)
fit <- fitted(dataknn)

k_nn <- data.frame(test_data$ID[1:297], fit[1:297])
names(k_nn) <- c("ID", "TotalDelay")
write.csv(k_nn, "kNN_Final_Top6.csv", row.names=FALSE)